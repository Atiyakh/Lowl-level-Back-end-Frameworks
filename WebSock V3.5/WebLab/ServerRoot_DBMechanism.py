...
class __Application_Database:
    ...
    def drop_db(self):
        f = open(os.path.join(os.getcwd(), 'WebApplication\\MiddleWare\\Database\\sqlite3.db'), 'wb')
        f.write(b''); f.close()
    def load_queries(self):
        ...
        f = open(os.path.join(os.getcwd(), f'WebApplication\\MiddleWare\\Database\\schemas\\schema_{self.padding_name(schema_count)}.sql'), 'r')
        data = f.read()
        f.close()
        # Slice:
        data = '["""' + data
...
class Model:
    ...
    def InitiateDatabase(self):
        try:
            self.db = sql.connect(os.path.join(os.getcwd(), 'WebApplication\\MiddleWare\\Database\\sqlite3.db'))
            cur = self.db.cursor()
